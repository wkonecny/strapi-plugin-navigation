...
- `isOverviewUiEnabled` - Enables the new management interface: an overview page listing all navigations with separate detail pages per navigation. Disabled by default, so the classic single-page interface stays active. Can also be toggled on the plugin Settings page.
